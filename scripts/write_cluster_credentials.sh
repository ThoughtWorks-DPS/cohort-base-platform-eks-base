...
aws sts assume-role --output json --role-arn arn:aws:iam::$AWS_ACCOUNT_ID:role/$AWS_ASSUME_ROLE --role-session-name cohort-base-platform-eks-base > credentials
...
cat kubeconfig_$CLUSTER | opw write cohort-base-platform-${CLUSTER} kubeconfig -

# terraform-aws-eks module v18 method
# terraform output kubeconfig | grep -v "EOT" | opw write cohort-base-platform-${CLUSTER} kubeconfig -

# write cluster url and pubic certificate to 1password
terraform output cluster_endpoint | tr -d \\n | sed 's/"//g' | opw write cohort-base-platform-${CLUSTER} cluster-endpoint -
terraform output cluster_certificate_authority_data | tr -d \\n | sed 's/"//g' | opw write cohort-base-platform-${CLUSTER} base64-certificate-authority-data -
